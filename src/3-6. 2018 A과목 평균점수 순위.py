...
df = pd.read_csv('./-Assignment-Project/data/2. 15-19년전체집계.csv', usecols=[0,1,2,4])
df.head()
...
fmdesc=fm2018.sort_values('A과목',ascending=False)
fmdesc

#------------------------------------------------------
sndesc = sn2018.sort_values('A과목',ascending=False)
sndesc
...
sndList
fmdesc['A과목']
plt.bar(fmdList,fmdesc['A과목'],label='신입생', color='r',width=0.5)
plt.bar(sndList,sndesc['A과목'],label='재학생', color='b',width=0.5)
plt.xticks(list(deDic.values()), labels=list(deDic.keys()), rotation=45, ha='right')

#신입생 학과 -> 숫자 0:기계, 1:화학

#재학생 학과 -> 0.3:기계, 위의 신입생 학과에 + 0.3
plt.title('2018학과별A과목점수')
plt.xlabel('학과')
plt.ylabel('A과목점수')
plt.legend()
plt.ylim([2.0,6.0])
